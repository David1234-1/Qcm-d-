...
class AuthManager {
  ...
  checkAuthState() {
    ...
    auth.onAuthStateChanged((user) => {
      if (!user) {
        // Si l'utilisateur n'est pas connecté, afficher la modal d'authentification
        this.showAuthModal();
      }
    });
  }
  ...
  setupEventListeners() {
    ...
    this.authOverlay?.addEventListener('click', (e) => {
      if (e.target === this.authOverlay) {
        // Empêcher la fermeture si l'utilisateur n'est pas connecté
        if (!this.currentUser) {
          return;
        }
        this.hideAuthModal();
  ...
  setupAuthStateListener() {
    ...
    auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        this.updateUI();
        this.hideAuthModal(); // Cacher la modal une fois connecté
        await this.syncUserData();
        NotificationManager.show('Connexion réussie !', 'success');
      } else {
        this.currentUser = null;
        this.updateUI();
        this.showAuthModal(); // Afficher la modal si déconnecté
